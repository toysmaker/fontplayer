...
const getBend = (start, end) => {
  ...
  const bendCursor = glyph.getParam('弯-弯曲游标')
  const bendDegree = glyph.getParam('弯-弯曲度')
...
const getComponents = (skeleton) => {
  ...
  let { corner: gou_control } = FP.getIntersection(
    { type: 'line', start: in_radius_end, end: in_gou_end },
    { type: 'line', start: out_gou_end, end: gou_vecter_end },
  )
  if (!FP.isPointOnLineSegment(gou_control, in_radius_end, in_gou_end)) {
