...
const getComponents = (skeleton) => {
  ...
  const out_radius_end = getRadiusPoint({
    ...
    radius: Math.min(out_radius * 2, FP.distance(out_radius_control, out_gou_end) * 0.5),
  })
  let in_wan_curves_final = in_radius_data.final_curves
  let out_wan_curves_final = out_radius_data.final_curves

  const end_p0 = out_radius_start
  const end_p3 = out_radius_end
  const end_p1 = FP.getPointOnLine(out_radius_start, out_corner_wan_gou, FP.distance(out_radius_start, out_corner_wan_gou) * 0.5)
  const end_p2 = FP.getPointOnLine(out_radius_end, out_corner_wan_gou, FP.distance(out_radius_end, out_corner_wan_gou) * 0.5)
  ...
  const in_wan_data = FP.getRadiusPointsOnCurve(FP.getCurvesPoints(in_wan_curves_final), radius)
  const in_wan_start_1 = in_wan_curves_final[0].start
  const in_wan_start_2 = in_wan_data.point
  in_wan_curves_final = in_wan_data.final_curves
  const out_wan_data = FP.getRadiusPointsOnCurve(FP.getCurvesPoints(out_wan_curves_final), radius)
  const out_wan_start_1 = out_wan_curves_final[0].start
  const out_wan_start_2 = out_wan_data.point
  out_wan_curves_final = out_wan_data.final_curves
  ...
  // 按逆时针方向绘制轮廓
  // 绘制左侧（内侧）侧轮廓
  pen.moveTo(out_wan_start_2.x, out_wan_start_2.y)
  pen.bezierTo(out_wan_start_1.x, out_wan_start_1.y, in_wan_start_1.x, in_wan_start_1.y, in_wan_start_2.x, in_wan_start_2.y)
  ...
  // 绘制轮廓连接线
  pen.lineTo(out_wan_start_2.x, out_wan_start_2.y)
