...
const getBend = (start, end) => {
  ...
  const bendCursor = glyph.getParam('捺-弯曲游标')
  const bendDegree = glyph.getParam('捺-弯曲度') + 30 * glyph.getParam('弯曲程度')
...
const getComponents = (skeleton) => {
  ...
  const end_length = Math.min(420, FP.distance(na_start, na_end) * 0.5)
  ...
  const out_tiao_start_radius_before = FP.getPointOnLine(out_tiao_start, tiao_start, FP.distance(out_tiao_start, tiao_start) * 0.8)
  const out_tiao_start_radius_after = FP.getPointOnLine(out_tiao_start, out_corner_tiao_na, radius)
  const in_tiao_start_radius_before = FP.getPointOnLine(in_tiao_start, tiao_start, FP.distance(in_tiao_start, tiao_start) * 0.8)
  const in_tiao_start_radius_after = FP.getPointOnLine(in_tiao_start, in_corner_tiao_na, radius)
  ...
  // 按逆时针方向绘制轮廓
  // 绘制横的右侧（内侧）轮廓
  pen.moveTo(out_tiao_start_radius_before.x, out_tiao_start_radius_before.y)
  pen.quadraticBezierTo(out_tiao_start.x, out_tiao_start.y, out_tiao_start_radius_after.x, out_tiao_start_radius_after.y)
  ...
  pen.lineTo(in_tiao_start_radius_after.x, in_tiao_start_radius_after.y)
  pen.quadraticBezierTo(in_tiao_start.x, in_tiao_start.y, in_tiao_start_radius_before.x, in_tiao_start_radius_before.y)

  // 绘制轮廓连接线
  pen.lineTo(out_tiao_start_radius_before.x, out_tiao_start_radius_before.y)
