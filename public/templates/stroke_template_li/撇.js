...
const getComponents = (skeleton) => {
  ...
  const end_length = Math.min(100, FP.distance(end, start) * 0.3)
  ...
  const start_p2_p4_vector = FP.turnAngleFromStart(start, in_pie_curves[0].start, startTopAngle, 100)
  ...
  const end_right_data = FP.getRadiusPointsOnCurve(
    FP.getCurvesPoints(start_right_data_2.final_curves),
    end_length,
    true,
  )
  ...
  const end_p0_joint = new FP.Joint(
    'end_p0',
    end_p0,
  )
  const end_p8_joint = new FP.Joint(
    'end_p8',
    end_p8,
  )
  const end_p7_joint = new FP.Joint(
    'end_p7',
    end_p7,
  )
  const end_p6_joint = new FP.Joint(
    'end_p6',
    end_p6,
  )
  const end_p1_joint = new FP.Joint(
    'end_p1',
    end_p1,
  )
  const end_p2_joint = new FP.Joint(
    'end_p2',
    end_p2,
  )
  const end_p3_joint = new FP.Joint(
    'end_p3',
    end_p3,
  )
  const end_p4_joint = new FP.Joint(
    'end_p4',
    end_p4,
  )
  const end_p5_joint = new FP.Joint(
    'end_p5',
    end_p5,
  )
