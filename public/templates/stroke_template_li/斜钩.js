...
const getBend = (start, end) => {
  ...
  const bendCursor = glyph.getParam('斜-弯曲游标')
  const bendDegree = glyph.getParam('斜-弯曲度') + 30 * glyph.getParam('弯曲程度')
...
const getComponents = (skeleton) => {
  ...
  // out指左侧（外侧）轮廓线
  // in指右侧（内侧）轮廓线
  const { out_xie_curves, out_xie_points, in_xie_curves, in_xie_points } = FP.getCurveContours('xie', { xie_start, xie_bend, xie_end }, _weight, {
    unticlockwise: true,   
  })
  ...
  const end_p1 = FP.getPointOnLine(out_radius_start, out_radius_control, FP.distance(out_radius_start, out_radius_control) * 0.5)
  const end_p3 = out_radius_end
  const end_p2 = FP.getPointOnLine(out_radius_end, out_radius_control, FP.distance(out_radius_end, out_radius_control) * 0.5)
  const end_p4_radius_before = FP.getPointOnLine(end_p4, end_p3, radius * 0.5)
  const end_p5_radius_after = FP.getPointOnLine(end_p5, in_radius_control, radius * 0.5)
  ...
  const start_length = Math.min(100, FP.distance(xie_start, xie_end) * 0.3)
  ...
  const start_p2_p3_vector = FP.turnAngleFromStart(xie_start, in_xie_curves_final[0].start, startTopAngle, 100)
  ...
  let out_xie_curves_final_1 = start_left_data_2.final_curves
