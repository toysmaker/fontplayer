...
const getComponents = (skeleton) => {
  ...
  const { out_gou_start, out_gou_end, in_gou_start, in_gou_end } = FP.getLineContours('gou', { gou_start, gou_end }, _weight_2, {
    startWeight: _weight_2,
    endWeight: _weight_2 * 0.25,
  })
  ...
  const out_radius_start_wan_gou = FP.getPointOnLine(
    out_corner_wan_gou,
    out_corner_zhe_wan,
    Math.min(out_radius_wan_gou * 2, FP.distance(out_corner_wan_gou, out_corner_zhe_wan) * 0.5),
  )
  const out_radius_end_wan_gou = getRadiusPoint({
    start: out_corner_wan_gou,
    end: out_gou_end,
    radius: Math.min(out_radius_wan_gou * 2, FP.distance(out_corner_wan_gou, out_gou_end) * 0.5),
  })

  const end_p5 = out_gou_end
  const end_p4 = in_gou_end
  const end_p0 = in_radius_start_wan_gou
  const end_p1 = FP.getPointOnLine(in_radius_start_wan_gou, in_corner_wan_gou, FP.distance(in_radius_start_wan_gou, in_corner_wan_gou) * 0.5)
  const end_p3 = in_radius_end_wan_gou
  const end_p2 = FP.getPointOnLine(in_radius_end_wan_gou, in_corner_wan_gou, FP.distance(in_radius_end_wan_gou, in_corner_wan_gou) * 0.5)
  const end_p4_radius_before = FP.getPointOnLine(end_p4, end_p3, radius)
  const end_p5_radius_after = FP.getPointOnLine(end_p5, out_corner_wan_gou, radius)
  ...
  const turn_length = 40 * turn_style_value
  ...
  const turn_p1_radius_before = FP.getPointOnLine(turn_p1, turn_p0, FP.distance(turn_p1, turn_p0) * 0.5)
  const turn_p1_radius_after = FP.getPointOnLine(turn_p1, turn_p2, FP.distance(turn_p1, turn_p2) * 0.4)
  const turn_p2_radius_after = FP.getPointOnLine(turn_p2, turn_p3, FP.distance(turn_p2, turn_p3) * 0.4)
  const turn_p2_radius_before = FP.getPointOnLine(turn_p2, turn_p1, FP.distance(turn_p2, turn_p1) * 0.5)

  const turn_p1_p2_middle = FP.getPointOnLine(turn_p1, turn_p2, FP.distance(turn_p1, turn_p2) * 0.5)
  ...
  // 绘制转角衬线
  pen.bezierTo(turn_p0.x, turn_p0.y, turn_p1.x, turn_p1.y, turn_p1_p2_middle.x, turn_p1_p2_middle.y)
