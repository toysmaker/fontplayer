...
const computeParamsByJoints = (jointsMap) => {
  ...
  const w2 = range(skeleton_9.x - skeleton_7.x, w2_range)
...
const updateGlyphByParams = (params, global_params) => {
  ...
  const skeleton_o = new FP.Joint('skeleton_0', {
    x: x0,
    y: y0,
  })
  const skeleton_0 = new FP.Joint('skeleton_0', {
    x: skeleton_o.x + w1 / 2,
    y: skeleton_o.y - h1 / 2 + h2,
  })
  ...
  glyph.addRefLine(refline(skeleton_1, skeleton_7))
...
const getComponents = (skeleton, global_params) => {
  ...
  // 根据骨架计算轮廓关键点
  const { skeleton_o, skeleton_0, skeleton_1, skeleton_2, skeleton_3, skeleton_4, skeleton_5, skeleton_6, skeleton_7, skeleton_8, skeleton_9 } = skeleton

  // out指上侧（外侧）轮廓线
  // in指下侧（内侧）轮廓线
  const stroke1_beziers = FP.getCircle(skeleton_o, r1 - weight / 2)
  const stroke2_beziers = FP.getCircle(skeleton_o, r1 + weight / 2)
  ...
  pen2.beginPath()
  pen2.moveTo(stroke2_beziers[stroke2_beziers.length - 1].end.x, stroke2_beziers[stroke2_beziers.length - 1].end.y)
  for (let i = stroke2_beziers.length - 1; i >= 0; i--) {
