...
const transformPoint = (
  ...
  let rootWidth = transformSequence[0].type === 'horizontal' ? 1000 - delta * 2 : 1000
  let rootX = transformSequence[0].type === 'horizontal' ? 0 + delta : 0

  for (let i = 0; i < transformSequence.length; i++) {
    const transformItem = transformSequence[i]
    const { startX, endX } = transformItem
    if (transformItem.type === 'horizontal') {
...
const computeTransformSequence = (match, parts, tree, options: ITransformOptions) => {
  ...
  if (parentOp === '⿲') {
    ...
    const gap0 = firstPartBounds.x - parentBounds.x
    const gap1 = secondPartBounds.x - firstPartBounds.x - firstPartBounds.width
    const gap2 = thirdPartBounds.x - secondPartBounds.x - secondPartBounds.width
    const gap3 = parentBounds.x + parentWidth - thirdPartBounds.x - thirdPartBounds.width
    const d = gap0 / parentWidth
    const e = gap1 / parentWidth
    const f = gap2 / parentWidth
    const g = gap3 / parentWidth
    ...
    if (lastPartIndex === 0) {
      ...
      startX = aaa + bbb + d + e + f
      endX = ccc + aaa + bbb + d + e + f
    }
  } else if (parentOp === '⿰') {
    const firstPartBounds = getPartBounds([...match.slice(0, -1), 0], parts, tree)
    const secondPartBounds = getPartBounds([...match.slice(0, -1), 1], parts, tree)
    ...
    const gap0 = firstPartBounds.x - parentBounds.x
    const gap1 = secondPartBounds.x - firstPartBounds.x - firstPartBounds.width
    const gap2 = parentBounds.x + parentWidth - secondPartBounds.x - secondPartBounds.width
    const d = gap0 / parentWidth
    const e = gap1 / parentWidth
    const f = gap2 / parentWidth
    const aa = Math.pow(a, options.params[0])
...
const updateHorizontalComponentsRatio = (originCharacters, characters, parameters) => {
...
export { updateHorizontalComponentsRatio }