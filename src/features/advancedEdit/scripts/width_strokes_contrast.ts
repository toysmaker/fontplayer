...
const transformPoint = (
  ...
  if (options.type === 'pow') {
    const ox = minX + width / 2
    const d = x - ox
    const D = Math.pow(Math.abs(d) * 2 / width, options.params[0])
    let newX = x
    if (D > thresholdInRatio && Math.abs(d) > threshold) {
      newX = ox + (d > 0 ? 1 : -1) * D * width / 2
    }
    return {
      name: point.name,
      x: newX,
...
const updateWidthStrokesContrast = (originCharacters, characters, parameters) => {
...
export { updateWidthStrokesContrast }