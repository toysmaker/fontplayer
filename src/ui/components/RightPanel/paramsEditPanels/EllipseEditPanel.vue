...
const ellipseFillColorDisplay = computed(() => {
  const v = (selectedComponent.value?.value as IEllipseComponent | undefined)?.fillColor
...
const clearEllipseFillColor = () => handleChangeFillColor(null)
</script>
          ...
          <n-form-item :label="t('panels.paramsPanel.fillColor.label')" :show-feedback="false">
            <div class="fill-color-row">
              <div class="params-fill-color-wrap">
                <n-color-picker
                  size="small"
                  :value="ellipseFillColorDisplay"
                  :show-alpha="true"
                  @update:value="handleChangeFillColor"
                />
              </div>
              <n-button size="small" quaternary @click="clearEllipseFillColor">
