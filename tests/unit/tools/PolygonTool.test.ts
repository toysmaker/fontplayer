...
import { PolygonTool } from '@/features/tools/polygon/PolygonTool'
...
describe('PolygonTool', () => {
  ...
  describe('单例模式', () => {
    it('should create instance with canvas and config', () => {
      const tool = PolygonTool.getInstance(canvas, config)
      expect(tool).toBeInstanceOf(PolygonTool)
      expect(tool.name).toBe('polygon')
    })

    it('should return same instance on subsequent calls', () => {
      const tool1 = PolygonTool.getInstance(canvas, config)
      const tool2 = PolygonTool.getInstance()
      expect(tool1).toBe(tool2)
    })

    it('should throw error if called without canvas/config on first call', () => {
      PolygonTool.reset()
      expect(() => PolygonTool.getInstance()).toThrow('canvas and config are required')
    })
  })
  ...
  describe('键盘事件', () => {
    it('should handle Enter key to close path', () => {
      const tool = PolygonTool.getInstance(canvas, config)
      tool.activate()
